...
>   - 任意精度的整数 (BigInt) ，可以安全地存储和操作大整数，甚至可以超过数字的安全整数限制。[ES2020](https://github.com/tc39/proposal-bigint) 引入了一种新的数据类型 BigInt，BigInt 从 V8 的 6.7 版本开始支持，对应的 Chrome 版本是 67。
>   - 字符串（String），字符串是一串表示文本值的字符序列，例如："Howdy" 。
>   - 代表（Symbol） ( 在 ECMAScript 6 中新添加的类型).。一种实例是唯一且不可改变的数据类型。
> - 以及对象（Object）。

**`typeof`运算符检查数据类型**  （最简单，不过Undefined和复杂数据类型都会输出为Object）

最好的判断类型的方法：使用`Object.prototype.toString.call`
 ...
 **hasOwnProperty()**  
	 ...
	 语法：对象.hasOwnProperty("属性名")   // eslint会报错

```javascript
Object.prototype.hasOwnProperty.call(obj, 'xxx') // 较安全，不会报错
```
...
| pop()        | 用来删除数组的最后一个元素，并返回被删除的元素               | 语法：数组.pop()                      |
| unshift()    | 向数组的开头添加一个或多个元素，并返回数组的新的长度         | 语法：数组.unshift(元素1,元素2,元素N) |
| shift()      | 删除数组的开头的一个元素，并返回被删除的元素                 | 语法：数组.shift()                    |
| reverse()    | 可以用来反转一个数组，它会对原数组产生影响                   | 语法：数组.reverse()                  |
| concat()     | 可以连接两个或多个数组，它不会影响原数组，而是新数组作为返回值返回 | 语法：数组1.concat(数组2)             |
| slice()      | 用数组的某个片段切出新数组，slice() 可接受两个参数，从开始参数选取元素，直到结束参数（不包括）为止。 | 语法：数组.reverse(开始参数,结束参数) |
| splice()     | 可以用来删除数组中指定元素，并使用新的元素替换，返回一个包含已删除项的数组 | 语法：数组.splice()                   |
| sort()       | 以字母顺序对数组进行排序                                     | 语法：数组.sort()                     |
